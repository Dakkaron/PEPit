...
#ifdef PROCESSOR_INCLUDE
  // Custom processor included from User_Setup (e.g., for Linux/SDL2)
...
int16_t TFT_eSPI::textWidth(const char *string, uint8_t font)
{
  ...
  if (font>1 && font<9) {
    char *widthtable = (char *)(uintptr_t)fontdata[font].widthtbl - 32; //subtract the 32 outside the loop
...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
#ifdef LOAD_RLE
  {
    if ((textfont>2) && (textfont<9)) {
      ...
      cwidth = pgm_read_byte( (uint8_t *)(uintptr_t)fontdata[textfont].widthtbl + uniCode-32 );
...
int16_t TFT_eSPI::drawChar(uint16_t uniCode, int32_t x, int32_t y, uint8_t font)
{
  ...
  int32_t width  = 0;
  int32_t height = 0;
  uintptr_t flash_address = 0;
  uniCode -= 32;

#ifdef LOAD_FONT2
  if (font == 2) {
    flash_address = (uintptr_t)chrtbl_f16[uniCode];
  ...
  {
    if ((font>2) && (font<9)) {
      flash_address = *(uintptr_t*)((uintptr_t)fontdata[font].chartbl + uniCode*sizeof(void*));
      width = pgm_read_byte( (uint8_t *)(uintptr_t)fontdata[font].widthtbl + uniCode );
