...
function GenTile(x, y, baseOdds)
  ...
  elseif (Stage == 5) then
    ...
    elseif (v < 22 and StrX == -1 and x<=5) then
      GenStructure(x, y, 2)
    elseif (v < 24 and StrX == -1 and x<=5) then
      GenStructure(x, y, 1)
    elseif (v < 27) then
      WorldMapArray[x][y] = 6
    elseif (v < 40) then
  ...
  elseif (Stage == 6) then
    ...
    elseif (v < 23 and StrX == -1 and x<=5) then
      GenStructure(x, y, 2)
    elseif (v < 25 and StrX == -1 and x<=5) then
      GenStructure(x, y, 1)
    elseif (v < 29) then
      WorldMapArray[x][y] = 6
    elseif (v < 40) then
  ...
  elseif (Stage == 7) then
    ...
    elseif (v < 25 and StrX == -1 and x<=5) then
      GenStructure(x, y, 2)
    elseif (v < 27 and StrX == -1 and x<=5) then
      GenStructure(x, y, 1)
    elseif (v < 33) then
      WorldMapArray[x][y] = 6
    elseif (v < 45) then
...
function InitTileFragment(dirtPath, overlayPath, flip)
  ...
  SetDrawTargetSprite(tmp)
  DrawSprite(overlay, 0, 0)
  SetDrawTargetSprite(tile)
  DrawSprite(tmp, 0, 0)
...
function InitTileFragments(tileTable, dirtPath)
  ...
  SetDrawTargetSprite(tileTable["shadowLeft"])
  DrawSprite(sFOWLeft, 0, 0)
  SetDrawTargetSprite(tileTable["shadowRight"])
  DrawSprite(sFOWRight, 31, 0)
  SetDrawTargetSprite(tileTable["shadowTop"])
  DrawSprite(sFOWTop, 0, 0)
  SetDrawTargetSprite(tileTable["shadowBottom"])
  DrawSprite(sFOWBottom, 0, 31)

  SetDrawTargetSprite(tileTable["shadowTopLeft"])
  DrawSprite(sFOWLeft, 0, 0)
  DrawSprite(sFOWTop, 0, 0)

  SetDrawTargetSprite(tileTable["shadowTopRight"])
  DrawSprite(sFOWRight, 31, 0)
  DrawSprite(sFOWTop, 0, 0)

  SetDrawTargetSprite(tileTable["shadowBottomLeft"])
  DrawSprite(sFOWLeft, 0, 0)
  DrawSprite(sFOWBottom, 0, 31)

  SetDrawTargetSprite(tileTable["shadowBottomRight"])
  DrawSprite(sFOWRight, 31, 0)
  DrawSprite(sFOWBottom, 0, 31)
  SetDrawTargetFramebuffer()
...
-- Progression Menu
function DisplayBuyStage(price)
  if (Money < price) then
