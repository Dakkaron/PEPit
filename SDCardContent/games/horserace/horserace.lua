...
if LeftHandedMode then
  if IsTouchInZone(0, 0, 50, 69) then
    DrawSprite(SJump, 27, 46, {angle=-0.5, flags=ALIGN_H_CENTER|ALIGN_V_CENTER})
  else
    DrawSprite(SJump, 5, 25)
  end

  if IsTouchInZone(0, 80, 50, 44) then
    DrawSprite(STurnLeft, 27, 101, {angle=-0.5, flags=ALIGN_H_CENTER|ALIGN_V_CENTER})
  else
    DrawSprite(STurnLeft, 5, 80)
  end

  if IsTouchInZone(0, 135, 50, 44) then
    DrawSprite(STurnRight, 27, 156, {angle=0.5, flags=ALIGN_H_CENTER|ALIGN_V_CENTER})
  else
    DrawSprite(STurnRight, 5, 135)
  end
else
  if IsTouchInZone(270, 0, 50, 69) then
    DrawSprite(SJump, 293, 46, {angle=-0.5, flags=ALIGN_H_CENTER|ALIGN_V_CENTER})
  else
    DrawSprite(SJump, 271, 25)
  end

  if IsTouchInZone(270, 80, 50, 44) then
    DrawSprite(STurnLeft, 293, 101, {angle=-0.5, flags=ALIGN_H_CENTER|ALIGN_V_CENTER})
  else
    DrawSprite(STurnLeft, 271, 80)
  end

  if IsTouchInZone(270, 135, 50, 44) then
    DrawSprite(STurnRight, 293, 156, {angle=0.5, flags=ALIGN_H_CENTER|ALIGN_V_CENTER})
  else
